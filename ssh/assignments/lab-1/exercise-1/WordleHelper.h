...
class WordleHelper{
    ...
    bool containsYellow(std::string& word, char c){
        for (int i = 0; i < word.length(); i++){
            if (word[i] == c){
                 word[i] = '-'; 
                return true;
...
public:
    ...
    std::vector<std::string>possibleSolutions(){
        ...
        for (int i = 0; i < yellow.size(); i++) {
            std::string currYellow = yellow[i];
            std::vector<std::string> wordsToRemove; // Create a vector to store words to be removed
            for (std::string& word : result) {
                bool satisfy = false;
                std::string copyWord = word;
                for (int i = 0; i < currYellow.size(); i++) {
                    char c = currYellow[i];
                    if (c != '_'){
                        if (containsYellow(copyWord, c) && word[i] != c) {
                            satisfy = true;
                        }else{
                            satisfy = false;
                            break;
                ...
                if (!satisfy) {
                    wordsToRemove.push_back(word); // Add the word to the removal list
                }
            }

            // Remove the words that need to be removed
            for (const std::string& wordToRemove : wordsToRemove) {
                result.erase(std::remove(result.begin(), result.end(), wordToRemove), result.end());
            }
        }
